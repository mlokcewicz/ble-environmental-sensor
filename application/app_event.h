...
struct app_event
{
    ...
    union
    {
        bool ble_connected;            // for APP_EVENT_BLE_CONNECTION_STATE_CHANGED
        bool led_state;                // for APP_EVENT_LBS_LED_SET_REQ
        uint32_t sampling_interval_ms; // for APP_EVENT_SAMPLING_INTERVAL_SET_REQ
        bool button_state;             // for APP_EVENT_LBS_BUTTON_STATE_CHANGED
        struct 
        {
        ...
        } sensor_data;                      // for APP_EVENT_SENSOR_DATA_UPDATE
    };
};
...
// MSQUEUE_DECLARE(sensor_event_queue, APP_EVENT_SENSOR_DATA_QUEUE_SIZE);
