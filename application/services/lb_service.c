...
struct lb_service_ctx 
{
    struct lb_service_cb lbs_cb;
...
LOG_MODULE_REGISTER(lb_service);

static struct lb_service_ctx ctx;
...
(   
    ...
    BT_GATT_CHARACTERISTIC(BT_UUID_LBS_LED, BT_GATT_CHRC_WRITE, BT_GATT_PERM_WRITE_AUTHEN, NULL, write_led, NULL),
	...
	BT_GATT_CHARACTERISTIC(BT_UUID_LBS_SAMPLING_INTERVAL, BT_GATT_CHRC_READ | BT_GATT_CHRC_WRITE, BT_GATT_PERM_READ | BT_GATT_PERM_WRITE_AUTHEN, read_sampling_interval, write_sampling_interval, &ctx.sampling_interval_ms),
);
...
int lb_service_send_button_state_indicate(bool button_state)
{
    ...
    ctx.ind_params.attr = &lb_svc.attrs[2];
...
int lb_service_send_sensor_notify(uint32_t sensor_value)
{
	...
	return bt_gatt_notify(NULL, &lb_svc.attrs[7], &sensor_value, sizeof(sensor_value));
}
